...
export default function WineForm({ wine = null }) {
  ...
  const getScanDefaults = () => {
    ...
    return {
      ...
      foodPairing: searchParams.get("foodPairing") || "",
      estimatedValue: searchParams.get("estimatedValue") || "",
      drinkFrom: searchParams.get("drinkFrom") || "",
      drinkUntil: searchParams.get("drinkUntil") || "",
    };
  };
    ...
    wine
      ? {
          ...
          purchaseDate: wine.purchaseDate
            ? new Date(wine.purchaseDate).toISOString().split("T")[0]
            : "",
  ...
  const removeAroma = (aroma) =>
    set(
      "aromaProfile",
      form.aromaProfile.filter((a) => a !== aroma),
    );

  const filteredSuggestions = AROMA_SUGGESTIONS.filter(
    (a) =>
      aromaInput.length > 0 &&
  ...
  const handleAIScan = (data) => {
    setForm((f) => ({
      ...
      ...(data.grapeVarieties && {
        grapeVarieties: Array.isArray(data.grapeVarieties)
          ? data.grapeVarieties.join(", ")
          : data.grapeVarieties,
      }),
      ...(data.foodPairing && {
        foodPairing: Array.isArray(data.foodPairing)
          ? data.foodPairing.join(", ")
          : data.foodPairing,
      }),
    ...
    setAiReport({
      identity:
        data.confidence?.identity ?? data.identification?.confidence ?? null,
      drinkWindow:
        data.confidence?.drinkWindow ?? data.drinkWindowConfidence ?? null,
      basis: data.drinkWindowBasis || null,
      ...
      <form
        ...
        {fromScan && (
          <div className="wf-scan-banner">
            <span>✨</span>
            <span>
              <strong>AI identified this wine.</strong> Câmpurile sunt sugestii
              editabile; verifică și corectează orice informație înainte de
              salvare.
              {form.drinkFrom && form.drinkUntil
                ? ` Drinking window estimated: ${form.drinkFrom}–${form.drinkUntil}.`
                : ""}
            </span>
            {aiReport && (
              <small>
                Identification confidence:{" "}
                {aiReport.identity == null
                  ? "unknown"
                  : `${Math.round(aiReport.identity * 100)}%`}{" "}
                · Drinking window:{" "}
                {aiReport.drinkWindow == null
                  ? "unknown"
                  : `${Math.round(aiReport.drinkWindow * 100)}%`}
              </small>
            )}
          </div>
        )}
        ...
        <div className="wf-mobile-header">
          <div className="wf-mobile-title-row">
            ...
            <span className="wf-mobile-count">
        ...
        <div className={`wf-section ${section === 0 ? "active" : ""}`}>
          ...
          <div className="wf-grid">
            <div className="wf-field wf-full">
              ...
              <input
                className={`wf-input ${errors.name ? "error" : ""}`}
            ...
            <div className="wf-field">
              <label className="wf-label">Culoare</label>
              <select
                className="wf-select"
                value={form.color}
            ...
            <div className="wf-field">
              ...
              <input
                className="wf-input"
                placeholder="ex. Bordeaux, Dealu Mare"
                value={form.region}
        ...
        <div className={`wf-section ${section === 1 ? "active" : ""}`}>
          <p className="wf-img-hint">
            🖼️ Adaugă imagini pentru a personaliza vinul în colecție. Poți
            încărca eticheta și/sau sticla.
          </p>
          <div className="wf-grid">
            <div className="wf-field">
              <ImageUpload
          ...
          <div className="wf-grid">
            ...
            <div className="wf-field">
              ...
              <input
                ...
                onChange={(e) => set("servingTemperature", e.target.value)}
              />
            </div>
            <div className="wf-field wf-full">
              ...
              <input
                ...
                value={form.grapeVarieties}
                onChange={(e) => set("grapeVarieties", e.target.value)}
              />
            </div>
            <div className="wf-field wf-full">
              ...
              <input
                className="wf-input"
                placeholder="ex. Vită, Brânzeturi maturate"
                value={form.foodPairing}
                onChange={(e) => set("foodPairing", e.target.value)}
              />
            </div>
            <div className="wf-field wf-full">
              ...
              <div className="wf-aroma-wrap">
                ...
                <input
                  ...
                  onKeyDown={(e) => {
                    ...
                    if (e.key === "," && aromaInput.trim()) {
                      e.preventDefault();
                      addAroma(aromaInput.replace(",", ""));
                    }
                  }}
                />
            ...
            <div className="wf-field wf-full">
              ...
              <label className="wf-check-wrap">
                ...
                />
                <div
                  className={`wf-check-box ${form.isFavorite ? "checked" : ""}`}>
                  {form.isFavorite && "✓"}
                </div>
        ...
        <div className={`wf-section ${section === 3 ? "active" : ""}`}>
          <div className="wf-grid">
            ...
            <div className="wf-field">
              ...
              <input
                className="wf-input"
                placeholder="ex. Raft A, Rândul 3"
                value={form.cellarLocation}
            ...
            <div className="wf-field wf-full">
              ...
              <div className="wf-drink-window">
                <div>
                  <label
                    className="wf-label"
                    style={{ fontSize: "0.62rem", marginBottom: "0.35rem" }}>
                    De la (an)
                  </label>
                  <input
                    className="wf-input"
              ...
              <div className="wf-stars">
                {[1, 2, 3, 4, 5].map((n) => (
                  <span
                    ...
                    onClick={() =>
                      set("rating", form.rating == n ? "" : n.toString())
                    }>
                    ★
                  </span>
                ))}
        ...
        <div className="wf-nav">
          ...
          <div className="wf-progress-dots">
            {SECTIONS.map((_, i) => (
              <div
                key={i}
                className={`wf-prog-dot ${i === section ? "active" : i < section ? "done" : ""}`}
              />
            ))}
          </div>
          {!isLastSection ? (

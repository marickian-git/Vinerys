...
// Arome predefinite grupate pe categorii
...
export default function WineForm({ wine = null }) {
  ...
  const addAroma = (aroma) => {
    ...
    if (!trimmed) return;
    if (form.aromaProfile.includes(trimmed)) return;
    set('aromaProfile', [...form.aromaProfile, trimmed]);
    setAromaInput('');
  };

  const removeAroma = (aroma) => {
    set('aromaProfile', form.aromaProfile.filter(a => a !== aroma));
  };

  const filteredSuggestions = AROMA_SUGGESTIONS.filter(a =>
    aromaInput.length > 0 &&
    a.includes(aromaInput.toLowerCase()) &&
    !form.aromaProfile.includes(a)
  );
  ...
  return (
    <>
      <style>{`
        ...
        .wform-stepper-header { display: flex; align-items: center; justify-content: space-between; margin-bottom: 0.85rem; }
        .wform-stepper-title { display: flex; align-items: center; gap: 0.5rem; font-size: 1rem; font-weight: 500; color: #f5e6e8; }
        .wform-stepper-count { font-size: 0.72rem; color: rgba(245,230,232,0.35); font-weight: 300; letter-spacing: 0.08em; }
        .wform-stepper-bar { height: 3px; background: rgba(196,69,105,0.15); border-radius: 2px; overflow: hidden; }
        .wform-stepper-fill { height: 100%; background: linear-gradient(90deg, #8b1a2e, #c44569); border-radius: 2px; transition: width 0.35s ease; }
        .wform-stepper-dots { display: flex; justify-content: center; gap: 6px; margin-top: 0.75rem; }
        .wform-stepper-dot { width: 28px; height: 4px; border-radius: 2px; background: rgba(196,69,105,0.2); transition: all 0.25s; cursor: pointer; border: none; padding: 0; }
        ...
        .wform-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 1.1rem; }
        ...
        .wform-input, .wform-select, .wform-textarea {
          ...
          border-radius: 8px; color: #f5e6e8; font-family: 'Jost', sans-serif;
          font-size: 0.85rem; font-weight: 300; outline: none; transition: all 0.25s ease;
        }
        ...
        .wform-error-msg { font-size: 0.68rem; color: rgba(220,80,80,0.8); margin-top: 0.35rem; }
        ...
        .wform-star { font-size: 1.4rem; cursor: pointer; transition: transform 0.15s ease; line-height: 1; color: rgba(245,230,232,0.15); }
        ...
        .wform-check-box {
          width: 18px; height: 18px; border: 1px solid rgba(196,69,105,0.4); border-radius: 4px;
        ...
        .wform-aroma-wrap:focus-within { border-color: rgba(196,69,105,0.5); box-shadow: 0 0 0 3px rgba(196,69,105,0.07); }
        .wform-aroma-tags { display: flex; flex-wrap: wrap; gap: 0.4rem; margin-bottom: 0.5rem; }
        .wform-aroma-tag {
          display: inline-flex; align-items: center; gap: 0.3rem;
          padding: 0.25rem 0.65rem; border-radius: 20px;
          background: rgba(196,69,105,0.12); border: 1px solid rgba(196,69,105,0.25);
          font-size: 0.75rem; color: rgba(245,230,232,0.8); font-weight: 300;
        }
        .wform-aroma-remove {
        ...
        .wform-aroma-remove:hover { color: #c44569; }
        .wform-aroma-input {
        ...
        .wform-aroma-input::placeholder { color: rgba(245,230,232,0.2); }
        .wform-aroma-suggestions {
          margin-top: 0.5rem; display: flex; flex-wrap: wrap; gap: 0.35rem;
        }
        .wform-aroma-suggestion {
        ...
        .wform-aroma-suggestion:hover { background: rgba(196,69,105,0.1); border-color: rgba(196,69,105,0.3); color: rgba(245,230,232,0.8); }
        ...
        .wform-drink-sep { font-size: 0.75rem; color: rgba(245,230,232,0.3); text-align: center; padding-top: 0.5rem; }
        ...
        .wform-btn-next {
          ...
          border: none; border-radius: 8px; color: #f5e6e8; font-family: 'Jost', sans-serif;
        ...
        .wform-prog-dot { width: 6px; height: 6px; border-radius: 50%; background: rgba(196,69,105,0.2); transition: all 0.2s; }
        ...
        @media (max-width: 640px) {
          ...
          .wform-drink-window { grid-template-columns: 1fr auto 1fr; }
          .wform-nav { flex-direction: column-reverse; gap: 0.65rem; padding-top: 1.25rem; }
          .wform-btn-back, .wform-btn-next { width: 100%; text-align: center; padding: 0.9rem 1rem; font-size: 0.82rem; }
      ...
      <form className="wform-wrap" onSubmit={handleSubmit}>
        ...
        {fromScan && (
          <div style={{
            ...
            borderRadius: '10px', fontSize: '0.75rem', color: 'rgba(212,175,55,0.9)',
          }}>
            <span>✨</span>
        ...
        <div className="wform-tabs">
          {SECTIONS.map((s, i) => (
            <button key={s.label} type="button"
              ...
              onClick={() => setSection(i)}>{s.label}</button>
          ))}
        </div>
        ...
        <div className="wform-stepper">
          ...
          <div className="wform-stepper-bar">
            <div className="wform-stepper-fill" style={{ width: `${((section + 1) / SECTIONS.length) * 100}%` }} />
          </div>
          <div className="wform-stepper-dots">
            {SECTIONS.map((s, i) => (
              <button key={i} type="button"
                className={`wform-stepper-dot ${i === section ? 'active' : i < section ? 'done' : ''}`}
                onClick={() => setSection(i)} aria-label={s.label} />
            ))}
          </div>
        </div>
        ...
        <div className={`wform-section ${section === 2 ? 'active' : ''}`}>
          <div className="wform-grid">
            ...
            {/* ── AROME ── */}
            <div className="wform-field wform-full">
              <label className="wform-label">Profil aromatic</label>
              <div className="wform-aroma-wrap">
                {form.aromaProfile.length > 0 && (
                  <div className="wform-aroma-tags">
                    {form.aromaProfile.map(a => (
                      <span key={a} className="wform-aroma-tag">
                        {a}
                        <button type="button" className="wform-aroma-remove" onClick={() => removeAroma(a)}>×</button>
                      </span>
                    ))}
                  </div>
                )}
                <input
                  className="wform-aroma-input"
                  placeholder={form.aromaProfile.length === 0 ? 'ex. cireșe, vanilie, piper negru...' : 'Adaugă aromă...'}
                  value={aromaInput}
                  onChange={e => setAromaInput(e.target.value)}
                  onFocus={() => setAromaFocused(true)}
                  onBlur={() => setTimeout(() => setAromaFocused(false), 150)}
                  onKeyDown={e => {
                ...
                {(aromaFocused && filteredSuggestions.length > 0) && (
                  <div className="wform-aroma-suggestions">
                    {filteredSuggestions.slice(0, 8).map(s => (
                      <button key={s} type="button" className="wform-aroma-suggestion"
        ...
        <div className={`wform-section ${section === 3 ? 'active' : ''}`}>
          <div className="wform-grid">
            ...
            {/* ── FEREASTRĂ DE CONSUM ── */}
            <div className="wform-field wform-full">
              <label className="wform-label">Fereastră optimă de consum</label>
              <div className="wform-drink-window">
                <div>
                  <label className="wform-label" style={{ fontSize: '0.62rem', marginBottom: '0.35rem' }}>De la (an)</label>
                  <input className="wform-input" type="number" placeholder={`ex. ${currentYear}`}
                    min="1900" max="2100" value={form.drinkFrom}
                    onChange={e => set('drinkFrom', e.target.value)} />
                </div>
                <div className="wform-drink-sep">—</div>
                <div>
                  <label className="wform-label" style={{ fontSize: '0.62rem', marginBottom: '0.35rem' }}>Până la (an)</label>
                  <input className="wform-input" type="number" placeholder={`ex. ${currentYear + 10}`}
